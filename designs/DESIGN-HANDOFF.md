# bf48db02-d0d2-4ff1-ad9d-f0d28a9d259d implementation handoff

This archive is the source of truth for turning the design into production code. Start from `evalit-prototype.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `evalit-prototype.html`
...
- A single self-contained `evalit-prototype.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `evalit-prototype.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `evalit-prototype.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
